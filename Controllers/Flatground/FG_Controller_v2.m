    ...
    methods (Access = protected)
        ...
        function [userInputs, Data] = stepImpl(obj, t, cassieOutputs, isSim, GaitLibrary, encoder_fil)
            ...
            if    t > 0.1 && obj.begin ==1
                ...
                % obj.gaitparams = ControlPolicy( obj, GaitLibrary, min(obj.dqx_b_fil,-0.5) );
                ...
                if obj.task == 1 % walking
                    ...
                    % hd -> desired, h0 -> current
                    % the output might be 
                    % 1-Abduction 2-Rotation <3-Leg pitch> <4-Leg length> 5-Toe
                    % <Outputs defined in the paper, need Inv to compute desired joint angles>
                    % Q: How h is defined in FORST? A: the joint angle, we have to convert it to output
                    ...
                    left_tune = - 1 * max([s_L; s_R]) * (obj.Kp_lateral_stand*(qroll - obj.roll_des_fil - (-lateral_shift)) + .25 * dqroll  );
                    right_tune =  1 * max([s_L; s_R]) * (obj.Kp_lateral_stand*(qroll - obj.roll_des_fil - (-lateral_shift)) + .25 * dqroll  );
                    ...
                    u_toe = - 20*(obj.Kp_toe_stand*obj.P_feedback_toe_fil + obj.Kd_toe_stand*( obj.com_vel_x_fil - 0));
